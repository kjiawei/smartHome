...
REDIS credis_connect(const char *host, int port, int timeout)
{
  ...
  if (inet_aton(host, &sa.sin_addr) == 0) {
  	 printf("inet_aton(host, &sa.sin_addr) == 0 \r\n");
...
{
  //int rc = cr_sendfandreceive(rhnd, CR_INT, "%s %s %zu  %s\r\n", 
   //                           cmd, key, strlen(member), member);
   
   // modified by yongming.li for new protocol
...
//added by yongming.li for hash set  begin
static int cr_hsetaddrem(REDIS rhnd, const char *cmd, const char *key, const char *member,const char * value)
{

   int rc = cr_sendfandreceive(rhnd, CR_INT, "%s %s  %s  %s \r\n", 
                              cmd, key, member,value);

  if (rc == 0 && rhnd->reply.integer == 0)
    rc = -1;
  printf("cr_hsetaddrem success rc is %d \r\n",rc);
  return rc;
}
int credis_hadd(REDIS rhnd, const char *key, const char *member,const char * value)
{
  return cr_hsetaddrem(rhnd, "HSET", key, member,value);
}
//added by yongming.li for hash set  end
