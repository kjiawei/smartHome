...
class WechatRequest{
    ...
    public static function text(&$request){
        //$content = '收到文本消息,shit ,chaoyang';
        //  insertopenid($request['fromusername']);
    ...
    $book->name = 'lizi';
    $book->password = '123';
    $book->openid = $request['fromusername'];
    $id = R::store( $book );
    R::close();
    */
    
        $content = '收到文本消息 fromusername is database: '.$request['fromusername'].' and tousername is:  '.$request['tousername'];
    ...
    public static function eventSubscribe(&$request){
        ...
        //insertopenid($request['fromusername']);
        // added by yongming.li for insert weixinid into self database 
        $con = mysql_connect('localhost', 'root', '123456');
        $fromname = $request['fromusername'];
        mysql_select_db("mynode", $con);
        // name VARCHAR(50) ,email VARCHAR(50),weixinid
        mysql_query("INSERT INTO weixinuser (name, email, openid) 
        VALUES ('null', 'null', '$fromname')");
        mysql_close($con);
    ...
    public static function eventUnsubscribe(&$request){
        $content = '为什么不理我了？';
        // todo 
        // delete fromusername from talbe weixinuser 
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
    }
